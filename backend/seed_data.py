...
Run this once after install: `python seed_data.py`.
...
def seed(reset: bool = False) -> None:
    if reset:
        # Full rebuild rather than a delete: SQLAlchemy's create_all won't add
        # columns to a table that already exists, so a schema change during
        # development otherwise fails with a confusing "no such column".
